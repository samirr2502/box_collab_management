...
def main():
    ...
    open("box_items.txt","w").close()
    open("colab.txt","w").close()
    with open("box_items.txt", "a", encoding="utf-8") as file:  # 'a' for append
        collaborators = []
        colab_file=open("colab.txt", "a", encoding="utf-8") 
        collaborations = client.folder(folder_id=item_ids[0]).get_collaborations()
        for collab in collaborations:
            target = collab.accessible_by
            
            if(target.name not in collaborators):
                collaborators.append(target.name)
                folder_name = client.folder(folder_id=item_ids[0]).get().name
        ...
        look_into_folders(client,file,colab_file,item_ids[0],collaborators)
    # for item in items:
